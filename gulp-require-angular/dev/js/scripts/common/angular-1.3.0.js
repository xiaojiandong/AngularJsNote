 ...
 *
 * @description
 * Extends the destination object `dst` by copying all of the properties from the `dev` object(s)
 * to `dst`. You can specify multiple `dev` objects.
 *
 * @param {Object} dst Destination object.
 * @param {...Object} dev Source object(s).
...
function shallowCopy(src, dst) {
  ...
  for(var key in src) {
    // shallowCopy is only ever called by $compile nodeLinkFn, which has control over dev
 ...
 *
 * To see the functional implementation check out dev/ngAnimate/animate.js
 */
 ...
 *   that contain interpolation (e.g. `dev="{{bar}}"`). Not only is this very efficient but it's also
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
  var hasDirectives = {},
      ...
      ALL_OR_NOTHING_ATTRS = makeMap('ngSrc,ngSrcset,dev,srcset');
   ...
   * @description
   * Retrieves or overrides the default regular expression that is used for whitelisting of safe
   * urls during img[dev] sanitization.
   *
   * The sanitization is a security measure aimed at prevent XSS attacks via html links.
   *
   * Any url about to be assigned to img[dev] via data-binding is first normalized and turned into
  ...
  this.$get = [
    ...
    Attributes.prototype = {
      ...
      $set: function(key, value, writeAttr, attrName) {
        ...
        // sanitize a[href] and img[dev] values
...
function $InterpolateProvider() {
  ...
  this.$get = ['$parse', '$exceptionHandler', '$sce', function($parse, $exceptionHandler, $sce) {
    ...
    function $interpolate(text, mustHaveExpression, trustedContext, allOrNothing) {
      ...
      // single expression be used for iframe[dev], object[dev], etc., we ensure that the value
...
function $RootScopeProvider(){
  ...
  this.$get = ['$injector', '$exceptionHandler', '$parse', '$browser',
     ...
     * ```html
     * <file dev="./test/ng/rootScopeSpec.js" tag="docs1" />
     * ```
     *
     * # Inheritance
...
var SCE_CONTEXTS = {
  ...
  // url.  (e.g. ng-include, script dev, templateUrl)
...
function $SceDelegateProvider() {
  ...
  this.$get = ['$injector', function($injector) {
     ...
     * contextual escaping contexts (such as ng-bind-html, ng-include, any dev
 ...
 *
 * Additionally, `a[href]` and `img[dev]` automatically sanitize their URLs and do not pass them
 ...
 * | `$sce.URL`          | For URLs that are safe to follow as links.  Currently unused (`<a href=` and `<img dev=` sanitize their urls and don't constitute an SCE context. |
 * | `$sce.RESOURCE_URL` | For URLs that are not only safe to follow as links, but whose contents are also safe to include in your application.  Examples include `ng-include`, `dev` / `ngSrc` bindings for tags other than `IMG` (e.g. `IFRAME`, `OBJECT`, etc.)  <br><br>Note that `$sce.RESOURCE_URL` makes a stronger statement about the URL than `$sce.URL` does and therefore contexts requiring values trusted for `$sce.RESOURCE_URL` can be used anywhere that values trusted for `$sce.URL` are required. |
...
function $SceProvider() {
  ...
  this.$get = ['$parse', '$sniffer', '$sceDelegate', function(
     ...
     * escaping contexts (such as ng-bind-html, ng-include, any dev attribute
 ...
 * http://stackoverflow.com/a/472729)  However, setting img[dev] does normalize the URL.
 * Unfortunately, setting img[dev] to something like "javascript:foo" on IE throws an exception.
 ...
 *
 * @description
 * Using Angular markup like `{{hash}}` in a `dev` attribute doesn't
 ...
 * ```html
 * <img dev="http://www.gravatar.com/avatar/{{hash}}"/>
 * ```
 *
 * The correct way to write it:
 * ```html
 * <img ng-dev="http://www.gravatar.com/avatar/{{hash}}"/>
 * ```
 *
 * @element IMG
...
// ng-dev, ng-srcset, ng-href are interpolated
    ...
    return {
      ...
      link: function(scope, element, attr) {
        ...
        attr.$observe(normalized, function(value) {
          ...
          // on IE, if "ng:dev" directive declaration is used and "dev" attribute doesn't exist
          // then calling element.setAttribute('dev', 'foo') doesn't do anything, so we need
 ...
 * @priority 400
 *
 * @param {string} ngInclude|dev angular expression evaluating to URL. If the source is a string constant,
 *                 make sure you wrap it in **single** quotes, e.g. `dev="'myPartialTemplate.html'"`.
 ...
 * @example
  <example>
    <file name="index.html">
      ...
      <div id="tpl-content" ng-include dev="currentTpl"></div>
    </file>
    <file name="protractor.js" type="protractor">
